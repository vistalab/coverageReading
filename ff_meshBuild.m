function [vw,newMeshNum] = ff_meshBuild(vw,hemisphere,p)
% Edited by RL (3/15) so that it can run with variable inputs as opposed to
% interacting with the GUI
%
% INPUTS
% vw                : the mrVista view
% hemisphere        : 'left' or 'right'
% p.leftMeshName    : name of left mesh
% p.rightMeshName   : name of right mesh
% p.smoothIters     : number of smooth interations
% p.smoothRelax     : relaxation parameter
%
...
if strcmpi(hemisphere,'left')
    hemiNum = 1; 
    meshfilename = p.leftMeshName;
elseif strcmpi(hemisphere,'right')
    hemiNum = 2; 
    meshfilename = p.rightMeshName;  
end
...
% Save mesh file
% [msh, savePath] = mrmWriteMeshFile(msh, filename, verbose)
[newMesh newMesh.path] = mrmWriteMeshFile(newMesh, meshfilename);
