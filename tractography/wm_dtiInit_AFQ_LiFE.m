...
for ii = list_subInds
    ...
    % subject's main dti data. move here
    ...
    % path with diffusion data
    dwiFile =  '/home/frk/2t1/fp96_data_variability/dt6_run01_fliprot__4mm/run01_fliprot_aligned_trilin_aligned_trilin.nii.gz';

    % the name of the connectome (fe) structure
    feFileName = ['fe_structure_',dt6BaseName];

    % folder to save LiFE outputs
    feSaveDir = fullfile(dirData, dt6BaseName, 'LiFE');

    % make this folder if it does not exist
    if ~exist(feSaveDir,'dir'), mkdir(feSaveDir); end

    display('Starting feConnectomeInit ----------------------------------'); 
    
    % Initialize a new connectome (fe) structure
    fe = feConnectomeInit(dwiFile, fg, feFileName, feSaveDir, dwiFile, t1Path);

    display('Starting feSet --------------------------------------------'); 
