...
%% don't modify here
tem.groupControls    = [1:10, 12:14]; 
tem.groupPoorReaders = 11;
tem.groupEveryone    = 1:14;
tem.groupCNI         = 10:14; 
tem.groupLucas       = 1:9; 
...
h.threshco      = 0;       % minimum of co
h.threshecc     = [.5 5];   % range of ecc
h.threshsigma   = [0 5];    % range of sigma
...
vfc.fieldRange      = 6;        % radius of stimulus
...
tem.saveFolderSub    = ['co' num2str(h.threshco) ...
    ...
    '.plotRad' num2str(vfc.fieldRange)]; 
...
if (~exist(tem.saveFolderGroup, 'dir') && tem.YNgroupAverage), mkdir(tem.saveFolderGroup), end
...
for jj = 1:length(tem.listRoi) %% looping over rois
    ...
    if tem.YNthreshold
        ...
        for ii = 1:length(tem.subjectsToAnalyze)
            ...
            if tem.passesThreshold
                ...
                % title with subject and roi
                title([RM_th{iisubRMth}.subject '. ' tem.roi], 'FontSize', 24)
                % description of thresholds on x-axis
                % laxis label is turned off somewhere in rmPlotCoveragefromROImatfile, so turn on
                tem.hxlab = get(gca,'XLabel'); 
                set(tem.hxlab, 'Visible', 'On'); 
                xlabel(tem.saveFolderSub,'FontSize',12)
                
                saveas(tem.figHandle, [tem.saveFolderSingle, tem.roi '_' RM_th{iisubRMth}.subject '.jpg'])
                
                %% stuff for later
                % record just the xs and ys, this is used later to plot the
                % centers
               RMcenters{tem.goodCounter}.x0 = RM_th{iisubRMth}.x0; 
               RMcenters{tem.goodCounter}.y0 = RM_th{iisubRMth}.y0;  
               
               % make a string of all subjects included
               tem.namestr = [tem.namestr '.' RM_th{iisubRMth}.subject]; 
               %%
            ...
            % if subject DOES NOT pass the threshold
                ...
                tem.fh = gcf;
                % title with subject and roi
                title([RM{iisub}.subject '. ' tem.roi], 'FontSize', 24)
                % description of thresholds on x-axis
                xlabel(tem.saveFolderSub,'FontSize',12)
    ...
    if ~tem.YNthreshold
        ...
        % initialize list of indices of good subjects 
        tem.indGoodList = []; 
...
%                 % add the centers of each person at a low alpha setting
%                 hold on
%                 for kk = 1:length(RMcenters)
%                     tem.x0 = RMcenters{kk}.x0;
%                     tem.y0 = RMcenters{kk}.y0; 
%                     % plot each prf center as a patch
%                     % (for now, only way to play with transparency)
%                     % will plot with flipped x values because
%                     % rmPlotCoveragefromROImatfile took care of that for us
%                     % consistency b/w subjects preserved
%                     for mm = 1:length(tem.x0)
%                         scatter(-tem.x0(mm), tem.y0(mm), ...
...
%                 
                ...
                % xlabel of plot: threshold limits and subjects involved.
                tem.hxlab           = get(gca,'Xlabel');
                set(tem.hxlab,'String', {tem.saveFolderSub; tem.namestr},'Visible','On')
