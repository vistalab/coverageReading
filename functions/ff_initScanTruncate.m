function [vw, scanNum, dtNum] = ff_initScanTruncate(vw, dataType, scanNum, src, timePointsToKeep)
% ACTUALLY THIS CODE WILL PROBABLY BE DELETED
...
if length(scanNum) > 1
    for i=1:length(scanNum)
        vw = ff_initScanTruncate(vw, dataType, scanNum(i), src);
    end
    return
end
