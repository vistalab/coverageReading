...
% path.knkResults = '/biac4/wandell/data/reading_prf/rosemary/20140425_1020/prfresultsSMALL2.mat';
...
% name that we want to give the averaged dataTYPE. 
% if this is already created, just input that same name here.
tem.name = 'Averages'; 

% datatype name to average over, if we don't have an average
% (load mrSESSION and see dataTYPES.name.
% go for the one that is motion and sliced-timed corrected)
tem.dtToAverage = 'BwScansMotionComp'; 


% paths and directories where we want the tranformed prf parameters to be stored
save_KnkRetMatFileDir    = ['/biac4/wandell/data/reading_prf/rosemary/20140425_1020/Gray/' tem.name '/'];
save_KnkRetMatFileName   = 'retmodel-knk2vista-prfresultsSMALL1.mat'; 

% if the dataTYPE of tem.name is already created, this is its dataTYPE
% number. If not, this is one more than the number of dataTYPES.
...
p_nFrames           = mrSESSION.functionals(tem.dtnumRet).nFrames; 
p_totalFrames       = mrSESSION.functionals(tem.dtnumRet).totalFrames; 
p_framePeriod       = mrSESSION.functionals(tem.dtnumRet).framePeriod;  
...
load([path.mrSESSION 'Gray/' tem.name '/knkrfsize.mat']); 
% store rfsize info in a vector
vecRfsize = map{1}; 

% load expt GRAY parameter map
load([path.mrSESSION 'Gray/' tem.name '/knkexpt.mat']); 
vecExpt = map{1}; 
...
load([path.mrSESSION 'Gray/' tem.name '/knkR2.mat']); 
% store in varexp
...
model{1}.rss = 1 - model{1}.varexp/100;
