        ...
        for jj = 1:length(list_roiNames)
            ...
            if rmExists && roiExists
                ...
                for cc = 1:length(list_contours)
                    ...
                    % ellipseMat is a 3D matrix with the ellipse colored
                    ellipseMat = ff_color_2Dinto3D(RFEllipseContour, colorEllipse, vfc.backgroundColor);
                    
                    % ellipseAndContourMat is also 3D
                    % outputMat = ff_color_2Dinto3D_overlaid(logical2D, trueColor, baseMat)
                    ellipseAndContourMat = ff_color_2Dinto3D_overlaid(RFcovContour, colorContour, ellipseMat);
                    
                    % ff_polarPlotFrom3DMatrix(rfcov, vfc)
                    ff_polarPlotFrom3DMatrix(ellipseAndContourMat, vfc);
                   
                    % title
                    titleName = {
                        'Fitted Ellipse with Contour';
                        infoString; 
                        mfilename;
                        };
